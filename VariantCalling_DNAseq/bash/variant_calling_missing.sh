#!/bin/bash
#SBATCH --job-name=variant_calling_missing
#SBATCH --nodes=1
#SBATCH -c 8
#SBATCH --mem=4000M
#SBATCH -p long
#SBATCH --output=variant_calling_missing.out
#SBATCH --error=variant_calling_missing.error
#SBATCH --mail-user=m.corteslopez@imb-mainz.de
#SBATCH --mail-type=ALL


source activate /home/mcortesl/anaconda2/envs/pacbio
module load samtools/1.3.1
module load jdk/1.8.0_181


totallines=$(wc -l missed.bc | awk '{print $1}')

i=0
while [ $i -le $totallines ]
	do
	awk -v st="$i" 'NR>=st && NR< st+100{print $0}' missed.bc > tmp100lines
	while read line; 
		do
			BC=$line
